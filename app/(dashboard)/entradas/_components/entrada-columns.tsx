...
function DeleteEntradaButton({
  ...
  const handleDelete = async () => {
    try {
      const res = await fetch(`/api/entradas/${publicCode}`, {
        method: "DELETE",
      });
      if (res.ok) {
      ...
      <AlertDialogContent className="bg-slate-800 border-slate-700">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="text-slate-400">
            Tem certeza que deseja excluir a entrada número{" "}
  ...
  return (
    <div className="flex items-center gap-1">
      <Button
        ...
        onClick={(e) => {
          e.stopPropagation();
          router.push(`/entradas/${row.publicCode}`);
        }}
...
export function getEntradaColumns(onRefresh: () => void): Column<EntryRow>[] {
  return [
    {
      key: "id",
      label: "Código",
