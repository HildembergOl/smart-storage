...
interface TabInfoEntradaProps {
  form: FormEntryProps;
  setForm: React.Dispatch<React.SetStateAction<FormEntryProps>>;
  itens: ItemEntrada[];
  setItens: React.Dispatch<React.SetStateAction<ItemEntrada[]>>;
}
...
export function TabInfoEntrada({
  form,
  setForm,
  itens,
  setItens,
  ...
  const isNew = id === "novo";
  const { enterpriseId: activeEnterpriseId, enterprisePublicCode, ready } = useApp();
  ...
  const handleDeleteItem = async (item: ItemEntrada) => {
    if (isNew) {
    ...
    if (!isNew) {
      try {
        const res = await fetch(`/api/entradas/${id}/items?itemId=${item.id}`, {
          method: "DELETE",
        });
  ...
  const handleAddItem = async () => {
    ...
    if (editingItemId) {
      ...
      if (!isNew) {
        try {
          const res = await fetch(
            `/api/entradas/${id}/items?itemId=${editingItemId}`,
            {
              method: "PATCH",
    ...
    if (!isNew) {
      try {
        const res = await fetch(`/api/entradas/${id}/items`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            productId,
        ...
        if (res.ok) {
          ...
          const data = await res.json();
          toast.error(data.error || "Erro ao adicionar item à entrada");
        }
      } catch {
        toast.error("Erro ao salvar item");
      }
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                type="number"
                className="bg-slate-900/50 border-slate-700 h-9 text-sm font-semibold text-emerald-400"
                id="ent-total-nfe"
                placeholder="0.00"
              />
            </div>
      ...
      {/* Itens da Entrada (Fica junto com o cabeçalho no mesmo arquivo) */}
      <Card className="bg-slate-800/40 border-slate-700/50">
        <div className="px-5 pt-5 pb-3 flex items-center justify-between">
          <h2 className="text-slate-200 font-semibold text-sm">Itens da Entrada</h2>
            ...
            <div className="flex items-center gap-3">
              <p className="text-slate-400 text-xs">
                ...
                <span className="text-sky-400 font-semibold">
                  {formatCurrency((novoItem.quantidade || 0) * (novoItem.valorUnit || 0))}
                </span>
              </p>
            </div>
          </div>
              ...
              <tbody>
                ...
                {itens.length === 0 && (
                  <tr>
                    ...
                    >
                      Nenhum item adicionado a esta entrada.
                    </td>
                  </tr>
                )}
              </tbody>
